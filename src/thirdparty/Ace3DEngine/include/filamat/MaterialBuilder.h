...
public:
    ...
    MaterialBuilder& customPostEffect(bool customPostEffect) noexcept;

    /**
     * 开启/关闭自定义着色，必须要在Lit模式下才能启用自定义着色。
     * 在mat文件中定义以下回调方法，该方法则是最终的着色函数，原 material 函数中的baseColor则不会起作用。
     * vec3 customShading(inout vec4 fragColor, MaterialInputs material) {
     *     ShadingData shadingData = getShadingData(material);
     *  // getPointLightData 函数必须在 HAS_DYNAMIC_LIGHTING 宏启用时才能调用（场景内有点光源时，该宏启用），其参数表示点光源的索引。
     *  #if defined(HAS_DYNAMIC_LIGHTING)
     *     LightData lightData = getPointLightData(0);
     *  #endif
     *
     *  // getDirectionalLightData 函数必须在 HAS_DIRECTIONAL_LIGHTING 宏启用时才能调用（场景内有平行光时，该宏启用）。
     *  #if defined(HAS_DIRECTIONAL_LIGHTING)
     *     LightData lightData = getDirectionalLightData();
     *  #endif
     *
     *     // 该颜色作为最终上屏色（如果开启了后处理，则还会经过后处理处理）
     *     fragColor = vec4(1.0, 0.0, 0.0, 1.0);
     * }
     */
    MaterialBuilder& customShading(bool customShading) noexcept;

    /**
     * 开启/关闭tone mapping。
     * 原逻辑：tone mapping在view配置，在所有物体渲染后做全屏后处理，会导致rt上的底图颜色也发生变化。
     * 现逻辑：view关闭color grading，在每个物体材质上开启tone mapping处理。
     * 使用自定着色 可调用方法 vec3 toneMapping(const vec3 x)
     * @param toneMapping 材质是否开启tone mapping
     * @return builder
     */
    MaterialBuilder& toneMapping(bool toneMapping) noexcept;

    MaterialBuilder& stencilState(const filament::backend::StencilState& stencilState) noexcept;
...
public:
    ...
    static constexpr size_t MAX_PARAMETERS_COUNT = 50;
...
private:
    ...
    bool mCustomPostEffect = false;

    bool mCustomShading = false;

    bool mToneMapping = false;

    filament::backend::StencilState mStencilState;
