...
class AssetLoader {
 public:
     ...
     * configuration.
     */
    FilamentAsset* createAssetFromHandle(const void* handle);
     ...
     * configuration.
     */
    FilamentAsset* createAssetFromHandle(const void* cgltf, std::vector<uint8_t>& glbdata);
     ...
     * configuration.
     */
    void* createCGLTFFromBinary(const uint8_t *bytes, uint32_t nbytes);
