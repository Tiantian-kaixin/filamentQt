...
public:
     ...
     * 返回存储材质参数的内存区域指针。
     */
     const void *getUniformInternalBuffer() noexcept;

    void setStencilTest(bool enable) noexcept;

    void setStencilFace(backend::StencilFace face) noexcept;

    backend::StencilStateDescriptor& getFrontStencilStateDescriptor() noexcept;
    const backend::StencilStateDescriptor& getFrontStencilStateDescriptor() const noexcept;

    backend::StencilStateDescriptor& getBackStencilStateDescriptor() noexcept;
    const backend::StencilStateDescriptor& getBackStencilStateDescriptor() const noexcept;
};
