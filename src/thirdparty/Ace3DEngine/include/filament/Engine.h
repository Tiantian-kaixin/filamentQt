...
public:
    ...
    /**
     * set the filamat path for engine init (just for iOS)
     * **/
    static void setFilamatPath(std::string filamatPath="");

    /**
     * 可以用于判断是否是sharedContext
     * @return mSharedGLContext
     * **/
    void* getSharedGLContext();
    ...
    static Engine* create(Backend backend, Platform* platform, 
            const backend::ContextOptions& glContext, bool enableMultiThread);
    ...
    std::vector<utils::Entity> cloneRenderable(const utils::Entity entity) noexcept;
     ...
     */
    void flushAndWait(bool waitFinished = true);
   ...
   /**
    ...
    // actually it's cpu memory size
    size_t getGLMemorySize();
